...
# Refresh only the rocq-runtime/stublibs entries: build-mathcomp.sh installs the
# patched elpi (wasm-safe digest, hash_bits=30) and rocq-elpi under the same
# overlay, and the engine must link THOSE (OCAMLPATH puts the overlay first).
mkdir -p "$OVERLAY"; rm -rf "$OVERLAY/rocq-runtime" "$OVERLAY/stublibs"
...
[ -d "$OVERLAY/elpi" ] && echo "   overlay has patched elpi/rocq-elpi (make mathcomp ran): the engine links them" \
                       || echo "   overlay has no patched elpi: run 'make mathcomp' before 'make real' for mathcomp support"
...
if [ -d "$NATIVE_COQLIB/theories" ]; then
  ...
  # user-contrib from the native build is only Ltac2, whose plugin is not linked
  # into the engine (a .vo whose plugin is absent fails with "Unknown dynamic
  # tag"), so it is not shipped; Stdlib and mathcomp are staged below.
  # The FULL Stdlib regenerated by the patched rocqc (web/build-stdlib.sh) under
  # logical name Stdlib, at user-contrib/Stdlib. stage-packs.sh splits it into one
  # lazy pack per top-level directory (Stdlib.ZArith, Stdlib.Reals, ...), so a
  # first import fetches only the directories it needs; no subset list to maintain.
  STDLIB_THEORIES="$WORK/stdlib-src/theories"
  if [ -d "$STDLIB_THEORIES/Reals" ]; then
    ( cd "$STDLIB_THEORIES" && find . -name '*.vo' | while read -r f; do
        mkdir -p "$HERE/dist/coqlib/user-contrib/Stdlib/${f%/*}"; cp "$f" "$HERE/dist/coqlib/user-contrib/Stdlib/$f"; done )
  fi
  grep -vE '^[[:space:]]*(archive|plugin)\(' "$SW/lib/rocq-runtime/META" > "$HERE/dist/coqlib/rocq-runtime.META"
  # packs.json (lazy-import manifest) + the mathcomp, Coquelicot and Equations
  # .vos if built + the native consistency probe (fails the build on a digest mismatch)
